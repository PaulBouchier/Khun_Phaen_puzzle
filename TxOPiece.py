...
# Two by One (horizontal) piece
class TxOPiece(Piece):
    def __init__(self, piece_name, start_x, start_y): 
        super().__init__(piece_name, 'TxO', start_x, start_y)
    ...
    def find_moves(self, mt1, mt2):
        ...
        if (mt1.y == self.y and mt1.x == self.x+2):
                edge = self.make_edge(self.x+1, self.y, 'R')
                rv.append(edge)
        if (mt2.y == self.y and mt2.x == self.x+2):
                edge = self.make_edge(self.x+1, self.y, 'R')
                rv.append(edge)
    ...
    def populate_grid(self, board_grid):
        board_grid[self.y][self.x] = 'P'
        board_grid[self.y][self.x+1] = 'P'
